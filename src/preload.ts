...
import { contextBridge, ipcRenderer, webUtils } from '@/platform/electron';
import { ADAPTER_BRIDGE_EVENT_KEY } from './adapter/constant';

/**
 * @description 注入到renderer进程�? 用于与main进程通信
 * */
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  // 获取拖拽文件/目录的绝对路�?/ Get absolute path for dragged file/directory
  getPathForFile: (file: File) => webUtils.getPathForFile(file),
  // 直接 IPC 调用（绕�?bridge 库）/ Direct IPC calls (bypass bridge library)
  ...
  // 修改密码不需要当前密�?/ Change password without current password
