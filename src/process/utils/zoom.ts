...
// 将输入的缩放因子限制在允许范围，避免异常�?/ Clamp zoom factor into safe range
...
// 获取当前全局缩放值（�?renderer 查询显示�? Expose current zoom for renderer state syncing
export const getZoomFactor = (): number => currentZoomFactor;

// 在新建窗口时应用最近一次缩放�?/ Apply stored zoom to a newly created window
export const applyZoomToWindow = (win: BrowserWindow): void => {
  win.webContents.setZoomFactor(currentZoomFactor);
};

// 将缩放同步到所有窗口，保持多窗口一�?/ Sync zoom factor across all BrowserWindows
...
// 设置绝对缩放值，记录并广播给所有窗�?/ Persist new zoom factor and broadcast to windows
...
// 在当前值基础上增量调整缩�?/ Adjust zoom by delta relative to current factor
