...
import { Avatar, Computer, Earth, Info, LinkCloud, Toolkit } from '@icon-park/react';
 ...
 */
export type SettingTab = 'account' | 'model' | 'agent' | 'tools' | 'webui' | 'system' | 'about';
 ...
 * ```
 */
const SettingsModal: React.FC<SettingsModalProps> = ({ visible, onCancel, defaultTab = 'account' }) => {
    ...
    const items: Array<{ key: SettingTab; label: string; icon: React.ReactNode }> = [
      {
        ...
        icon: <Avatar theme='outline' size='20' fill={iconColors.secondary} />,
      },
      {
        key: 'model',
  ...
  const renderContent = () => {
    switch (activeTab) {
      case 'account':
        return <AccountModalContent onRequestClose={onCancel} />;
      case 'model':
