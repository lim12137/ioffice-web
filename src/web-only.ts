...
const loadUserWebUIConfig = (): { config: WebUIUserConfig; path: string | null; exists: boolean } => {
  try {
    // 优先检查项目根目录的配置文件
    // Priority: Check project root config file first
    const projectRoot = process.cwd();
    const projectConfigPath = path.join(projectRoot, WEBUI_CONFIG_FILE);

    if (fs.existsSync(projectConfigPath)) {
      const raw = fs.readFileSync(projectConfigPath, 'utf-8');
      const parsed = JSON.parse(raw) as WebUIUserConfig;
      if (parsed && typeof parsed === 'object') {
        return { config: parsed, path: projectConfigPath, exists: true };
      }
    }

    // 如果项目内不存在，检查用户数据目录
    // If not found in project, check user data directory
    const userDataPath = app.getPath('userData');
    const userConfigPath = path.join(userDataPath, WEBUI_CONFIG_FILE);
    if (fs.existsSync(userConfigPath)) {
      const raw = fs.readFileSync(userConfigPath, 'utf-8');
      const parsed = JSON.parse(raw) as WebUIUserConfig;
      if (parsed && typeof parsed === 'object') {
        return { config: parsed, path: userConfigPath, exists: true };
      }
    }

    return { config: {}, path: projectConfigPath, exists: false };
  } catch {
